...
**通信模式**：MCP官方定义stdio（标准输入输出）与Streamable HTTP（远程服务）两种标准传输，早期的HTTP/SSE传输已被Streamable HTTP取代；部分实现另支持WebSocket（双向通信）等扩展方式。
...
MCP的标准传输有两种。stdio模式通过标准输入输出进行通信，适合本地工具集成，Client和Server在同一台机器上运行。Streamable HTTP模式通过HTTP协议进行通信，适合远程服务调用，支持流式响应与服务器推送（取代了早期的HTTP/SSE传输）。WebSocket模式提供双向通信能力，适合需要实时交互的扩展场景。
...
**问题一：通信模式选择**
stdio模式适合本地集成，但不支持远程调用；Streamable HTTP适合远程调用与请求-响应场景（支持流式推送）；WebSocket适合需要实时双向通信的扩展场景。需要根据实际需求选择合适的模式。
