...
# 执行查询
result = agent.invoke({"input": "北京今天天气怎么样？请计算一下15+25的结果"})
print(result["output"])
```
...
> **迁移说明**：`initialize_agent` 在 v1.x 中已废弃，推荐使用 `create_agent`。
...
# 执行
result = agent.invoke({"input": "今天的日期是什么？请查找关于Python的信息"})
print(result["output"])
```
...
> **迁移说明**：`initialize_agent` 和 `AgentType.PLAN_AND_EXECUTE` 在 v1.x 中已废弃，推荐使用 `create_agent` 结合 LangGraph 实现复杂任务规划。
...
> **注意**：v1.x 中所有 Agent 类型统一通过 `create_agent` 创建，旧版的 `AgentType` 枚举和 `initialize_agent` 已废弃。

## 7 Agent 执行（v1.x 推荐方式）

### 7.1 执行流程详解

在 v1.x 中，`create_agent` 替代了旧版的 `AgentExecutor` + `create_openai_functions_agent` 组合，内部基于 LangGraph 自动管理推理-执行循环。

> **迁移说明**：`AgentExecutor` 在 v1.x 中已被 `create_agent` 取代。`create_agent` 内部使用 LangGraph 管理执行循环，无需手动配置 `max_iterations` 等参数。
...
`create_agent` 支持丰富的回调机制，用于监控和干预执行过程。
...
# 测试
try:
    result = agent.invoke({"input": "执行命令 'test'"})
    print(result["output"])
except Exception as e:
    print(f"执行异常: {e}")
```
...
# 测试不同类型的查询
result1 = agent.invoke({"input": "如何在Python中使用装饰器？"})
print(result1["output"])
print("\n" + "="*50 + "\n")
result2 = agent.invoke({"input": "查找langchain agent的示例代码"})
...
# 4. 合理设置超时
# 通过 invoke 的超时参数控制
# result = agent.invoke({"input": "问题"}, timeout=120)

# 5. 使用流式输出提升用户体验
# v1.x 支持流式输出，streaming node name 为 "model"
...
4. **错误处理**：Tool应该能优雅地处理错误，create_agent 内部基于 LangGraph 提供错误处理机制
...
> **重要迁移说明**：
> - `AgentExecutor` + `create_openai_functions_agent` 已被 `create_agent` 取代
> - `initialize_agent` 在 v1.x 中已废弃
> - `create_agent` 从 `langgraph.prebuilt` 移至 `langchain.agents`
> - Agent 内部基于 LangGraph，streaming node name 为 `"model"`