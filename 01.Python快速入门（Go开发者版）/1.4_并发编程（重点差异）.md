...
- **Python**: 默认构建受GIL限制，多线程不能真正并行（3.13+可选free-threaded构建）；asyncio是单线程事件循环；多进程可以并行但开销大
...
**定义**: GIL是CPython解释器的互斥锁，确保同一时刻只有一个线程执行Python字节码。PEP 703 free-threaded构建（3.13实验性、3.14起正式支持）可关闭GIL，但默认构建仍带GIL。
...
| "移除GIL就能加速" | 会牺牲单线程性能，破坏C扩展兼容性（PEP 703 free-threaded构建已可选关闭GIL，默认构建仍是此权衡） |
