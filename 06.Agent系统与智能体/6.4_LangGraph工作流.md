...
> **LangChain v1.x 更新提示**：Agent 创建已统一为 `create_agent`（从 `langchain.agents` 导入），取代了旧的 `AgentExecutor`、`create_openai_functions_agent` 和 `initialize_agent`。旧功能已迁移至 `langchain-classic`。LangGraph 新增了 Middleware 架构支持 `before_model`、`after_model`、`wrap_tool_call` 等钩子。
