...
- **LangChain** 是最流行的LLM应用开发框架，提供了构建Agent的完整工具链，支持多种推理模式
...
- 支持多种类型：Zero-shot、Conversational、ReAct等
...
**Q3: 记忆系统内存占用过大怎么办？**
- 使用`ConversationBufferMemory`并设置最大长度
- 定期清理过期的对话历史
- 考虑使用摘要记忆（Summary Memory）

**Q4: 如何自定义Tool？**
- 继承`BaseTool`类
- 实现`name`、`description`属性和`run`方法
- 在Agent初始化时传入自定义工具列表
...
1. **初学者入门**：推荐从LangChain开始，学习Agent基本概念
