...
EDA（Easy Data Augmentation）是由Jason Wei与Kai Zou于2019年提出的一种简单有效的文本数据增强方法。EDA包含四种基本操作：同义词替换、随机插入、随机交换和随机删除。这四种操作可以独立使用，也可以组合使用。EDA的核心思想是通过对文本进行简单的随机变换，在保持语义基本不变的前提下生成多样化的增强样本。

EDA的优点是实现简单、不依赖预训练模型，只需一份同义词词典即可在任何语言的文本上直接应用。缺点是变换后的文本可能不够流畅，需要控制增强比例以避免引入过多噪声。
