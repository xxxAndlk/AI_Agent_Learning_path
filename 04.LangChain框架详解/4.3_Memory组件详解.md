...
> **v1.x 迁移说明**：上述 Memory 类属于 v0.x 旧范式，在 v1.x 中已随 langchain-classic 进入维护模式。v1.x 推荐用 `ChatMessageHistory`/`RunnableWithMessageHistory`（LCEL 链）或 `create_agent` + LangGraph `checkpointer`（Agent）实现对话记忆；本章后续对各类旧 Memory 类的讲解保留作迁移对照。
...
在 v1.x 中推荐用 LCEL + RunnableWithMessageHistory 集成Memory（旧版 LLMChain 的 memory 参数已随 langchain-classic 进入维护模式）：
...
在LangChain v1.x中，可以通过 `create_agent()` 的 `checkpointer` 参数（LangGraph 状态持久化）或 Middleware 方式为 Agent 集成 Memory；LCEL 链则推荐 `RunnableWithMessageHistory`：
