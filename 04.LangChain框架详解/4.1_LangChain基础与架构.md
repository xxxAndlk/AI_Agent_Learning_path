...
**LangChain v1.x 版本特性**（截至2026年9月，最新版 `langchain-core>=1.2.31`）：
...
- **v1.x（2026年4月，最新稳定版 1.2.31）**：中间件系统、生产级工具链
...
v1.0: LangGraph Agent-centric（以 LangGraph 为底层的智能体为中心）
       ↓
v1.x: Middleware-centric + Production-ready
```
...
将多个组件按顺序组合，形成端到端的处理流程。在v1.x中，推荐使用 `create_agent()` 与 Middleware 架构，底层基于 LangGraph。

**传统Chain**（v0.x 旧范式，v1.x 中已移至 langchain-classic 维护模式，仅作迁移对照）：
```python
...
- **`create_agent`**（v1.x推荐）：一键创建基于 LangGraph 的生产级 Agent
...
LangChain v1.x 引入的生产级扩展机制，允许在 Agent 循环的关键节点注入自定义逻辑。Middleware 架构本身是真实的，但不存在内置的具体类，需要开发者自行实现：

- **`before_model`**：在 LLM 调用前修改消息或系统状态
- **`after_model`**：在 LLM 返回后处理响应内容
- **`wrap_tool_call`**：包装工具调用，可在执行前后注入逻辑

> 注意：具体 Middleware 类（如 SummarizationMiddleware、HumanInTheLoopMiddleware、RateLimitMiddleware 等）并不存在内置实现，需要开发者基于 Middleware 架构自行开发。

### 3.7 Runtime

- **Runtime**：Agent 的运行时环境，管理状态持久化、消息路由、流式传输
- 状态持久化基于 LangGraph 的内置能力，支持断点恢复和人机协作
...
│  │(before_model/│  │   Storage    │  │ (Observability)│     │
│  │after_model/..)│  │   (State)    │  │              │      │
│  └──────────────┘  └──────────────┘  └──────────────┘      │
└─────────────────────────────────────────────────────────────┘
```
...
### Q1: LangChain v0.x 和 v1.x 有什么区别？

**主要差异**：
- **v0.x**: 基于`Chain`类，继承复杂，调试困难
- **v1.x**: 基于`Runnable`接口和LCEL，更灵活、类型安全；Agent使用`create_agent`替代了`AgentExecutor`
...
| 场景 | 推荐模型 | 说明 |
|------|---------|------|
| 简单问答 | gpt-5.4-mini | 成本低，速度快 |
| 复杂推理 | gpt-5.4 | 能力强，准确率高 |
| 长文本处理 | gpt-5.4 | 128K上下文 |
| 代码生成 | Claude-4-Opus | 代码能力突出 |
| 本地部署 | Llama-3.1-70B | 隐私敏感场景 |
...
**技术栈**：
- create_agent + RunnableWithMessageHistory
- Memory维护会话状态
- Fallback到人工客服
- 集成企业工单系统
