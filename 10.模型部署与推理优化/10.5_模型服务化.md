...
# 31. 初始化注册模型
model_manager.register_model("gpt-4o-mini", MockLLM("gpt-4o-mini"))
...
if __name__ == "__main__":
    ...
    # 60. 模型路由
    router = ModelRouter()
    router.register_backend("gpt-4o-mini", "http://localhost:8001", weight=1.0)
    router.register_backend("gpt-4", "http://localhost:8002", weight=0.5)
    
    endpoint = router.route("gpt-4o-mini")
    print(f"\n路由到: {endpoint}")
```
