...
class ModelManager:
    ...
    def __init__(self):
        ...
        self.default_model = "gpt-3.5-turbo"   # 默认模型
...
# 31. 初始化注册模型
model_manager.register_model("gpt-3.5-turbo", MockLLM("gpt-3.5-turbo"))
model_manager.register_model("gpt-4", MockLLM("gpt-4"))
...
if __name__ == "__main__":
    ...
    # 60. 模型路由
    router = ModelRouter()
    router.register_backend("gpt-3.5-turbo", "http://localhost:8001", weight=1.0)
    router.register_backend("gpt-4", "http://localhost:8002", weight=0.5)
    
    endpoint = router.route("gpt-3.5-turbo")
    print(f"\n路由到: {endpoint}")
```
