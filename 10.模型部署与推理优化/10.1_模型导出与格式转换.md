...
**ONNX导出实现**：
使用torch.onnx.export函数进行导出，关键参数包括：model（模型）、example_input（示例输入）、save_path（保存路径）、opset_version（算子集版本）、input_names/output_names（输入输出名称）、dynamic_axes（动态维度）。opset_version选择要考虑目标运行时的支持情况，PyTorch 2.x默认导出opset 17/18。
